    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center px-5">
      <div className="w-full max-w-sm bg-white rounded-3xl shadow-sm border border-gray-100 p-7 text-center">
        {loading ? (
          ...
          <>
            <p className="text-[15px] font-semibold text-gray-900">Invite unavailable</p>
          ...
          <>
            ...
            <p className="text-[12px] font-semibold uppercase tracking-widest text-violet-500 mb-1">Team board invite</p>
            <h1 className="text-[20px] font-bold text-gray-900">{board.name}</h1>
            <p className="text-[13px] text-gray-500 mt-1.5">
              {board.owner_name ? <>Shared by <b>{board.owner_name}</b> · </> : null}{board.member_count} member{board.member_count !== 1 ? 's' : ''}
            </p>
            <p className="text-[13px] text-gray-600 mt-4">Join to view and add links together — it's free for members.</p>
            ...
            <button onClick={handleJoin} disabled={joining}
              ...
              {signedIn ? (joining ? 'Joining…' : 'Join board') : 'Sign in to join'}
            </button>
          </>
        ) : null}
      </div>
