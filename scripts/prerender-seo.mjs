...
function guideHtml(guide, otherLang) {
  ...
  const boardCta = guide.boardUrl
    ? `<aside aria-label="${boardKo ? '광고' : 'Advertisement'}" style="margin:24px 0;padding:20px;border:2px solid #e9d5ff;border-radius:16px;background:#faf5ff;max-width:680px">
        ...
        <p style="margin:0"><a href="${escAttr(withGuideSrc(guide.boardUrl, `${guide.slug}-${guide.lang}`))}" style="display:inline-block;padding:12px 24px;background:linear-gradient(90deg,#A259FF,#FF7262);color:#fff;text-shadow:0 1px 2px rgba(0,0,0,.35);border-radius:12px;font-weight:700;text-decoration:none">${boardKo ? '모든 링크 한곳에서 열기' : 'Open all the links in one place'} →</a></p>
      </aside>`
    : '';
...
function promoHtml(guide) {
  ...
  return `${css}<aside aria-label="${guide.lang === 'ko' ? '광고' : 'Advertisement'}" style="margin:24px 0;max-width:680px">
        <a class="pvp" href="${escAttr(withGuideSrc(guide.promoUrl, `${guide.slug}-${guide.lang}`))}" aria-label="${escAttr(guide.promoCta)}"
